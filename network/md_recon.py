...
class MRIReconstruction(nn.Module):
    def __init__(self, bn = True, isFastmri=False):
        ...
        if isFastmri:
            dc_temp = DC_fastmri
            self.fft_temp = fft_fastmri
            self.ifft_temp = ifft_fastmri
        else:
            dc_temp = DC
            self.fft_temp = fft
            self.ifft_temp = ifft
    ...
    def forward(self, *input):
        ...
        k_to_img_1 = self.ifft_temp(rec_k_1)  # convert the restored kspace to spatial domain
        img_to_k_1 = self.fft_temp(rec_img_1) # convert the restored image to frequency domain
        ...
        k_to_img_2 = self.ifft_temp(rec_k_2)  # convert the restored kspace to spatial domain
        img_to_k_2 = self.fft_temp(rec_img_2) # convert the restored image to frequency domain
        ...
        k_to_img_3 = self.ifft_temp(rec_k_3)  # convert the restored kspace to spatial domain
        img_to_k_3 = self.fft_temp(rec_img_3) # convert the restored image to frequency domain
        ...
        k_to_img_4 = self.ifft_temp(rec_k_4)  # convert the restored kspace to spatial domain
        img_to_k_4 = self.fft_temp(rec_img_4) # convert the restored image to frequency domain
        ...
        k_to_img_5 = self.ifft_temp(rec_k_5)  # convert the restored kspace to spatial domain
