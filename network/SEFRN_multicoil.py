"""
This file contains the official implementation of SEFRN (multi-coiled version)
"""
...
class WADCRG(nn.Module):
    ...
    def __init__(self, inChannel = 2, wChannel = 32, fmChannel=64, kernel_size = 3, n_resblocks=5, act= nn.ReLU(True), type_tdcfm=0, isFastmri=False, isMulticoil=False):
        ...
        self.dcrg = DCRG(inChannel, fmChannel, kernel_size, act, n_resblocks, isFastmri=isFastmri, isMulticoil=isMulticoil)
...
class SEFRN_multicoil(nn.Module):
    def __init__(self, inChannel = 30, 
                wChannel = 64, \
                fmChannel = 32, \
                skChannel = 48, \
                c=3,\
        ...
        subgroups = 5 
        kernel_size = 3
        n_resblocks : number of resblocks
        c: recurrent time 
        """

        super(SEFRN_multicoil, self).__init__()
        self.recur_times = c
        layers = []
