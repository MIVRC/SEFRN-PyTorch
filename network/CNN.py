...
class Unet_dc_multicoil(nn.Module):
    def __init__(self, indim=30, isFastmri=True, isMulticoil=True):
        
        super(Unet_dc_multicoil, self).__init__()
        self.block0 = nn.Sequential()
        self.block0.add_module("conv_0", nn.Conv2d(indim,72,3,padding = 1))
        self.block0.add_module("relu_0", nn.ReLU())
        
        self.block1 = nn.Sequential()
        self.block1.add_module("conv_1", nn.Conv2d(72,72,3,padding = 1))
        self.block1.add_module("relu_1", nn.ReLU())
        
        self.down = nn.Sequential()
        self.down.add_module("maxpool_d", nn.MaxPool2d(kernel_size=2))
        self.down.add_module("conv_d1", nn.Conv2d(72,144,3,padding = 1))
        self.down.add_module("relu_d1", nn.ReLU())
        self.down.add_module("conv_d2", nn.Conv2d(144,144,3,padding = 1))
        self.down.add_module("relu_d2", nn.ReLU())

        self.middle = nn.Sequential()
        self.middle.add_module("maxpool_m", nn.MaxPool2d(kernel_size=2))
        self.middle.add_module("conv_m1", nn.Conv2d(144,288,3,padding = 1))
        self.middle.add_module("relu_m1", nn.ReLU())
        self.middle.add_module("conv_m2", nn.Conv2d(288,288,3,padding = 1))
        self.middle.add_module("relu_m2", nn.ReLU())
        self.middle.add_module("conv_m3", nn.Conv2d(288,144,3,padding = 1))
        self.middle.add_module("relu_m3", nn.ReLU())
        self.middle.add_module("upsampling_m", nn.ConvTranspose2d(144,144,2,2))
        
        self.up = nn.Sequential()
        self.up.add_module("conv_s1", nn.Conv2d(288, 144, 1, padding= 0))
        self.up.add_module("relu_s1", nn.ReLU())
        self.up.add_module("conv_s2", nn.Conv2d(144,72, 1, padding= 0))
        self.up.add_module("relu_s2", nn.ReLU())
        self.up.add_module("upsampling_u", nn.ConvTranspose2d(72,72,2,2))
        
        self.block2 = nn.Sequential()
        self.block2.add_module("conv_s3", nn.Conv2d(144, 72, 1, padding= 0))
        self.block2.add_module("relu_s3", nn.ReLU())
        self.block2.add_module("conv_s4", nn.Conv2d(72, 72, 1, padding= 0))
        self.block2.add_module("relu_s4", nn.ReLU())
        self.block2.add_module("conv_5", nn.Conv2d(72, indim, 1, padding=0))
