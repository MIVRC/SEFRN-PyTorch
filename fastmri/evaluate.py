...
from .data import transforms_simple
...
def rmse(gt: np.ndarray, pred: np.ndarray) -> np.ndarray:
    """Compute Mean Squared Error (MSE)"""
...
class Metrics:
    ...
    def push(self, target, recons, maxvals=None):
        for metric, func in METRIC_FUNCS.items():
            self.metrics[metric].push(func(target, recons, maxvals))
